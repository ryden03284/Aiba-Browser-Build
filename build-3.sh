...
# Resolve which branch to use.  Priority:
#   1. The user-supplied / env-var DEBIAN_BRANCH (if it actually exists remotely)
#   2. The remote HEAD (true default branch, e.g. "bookworm" for uc-debian)
#   3. Hard failure with a list of available remote branches to help the user
#
# Sets DEBIAN_BRANCH to the resolved name so downstream functions see the
# correct value.
_resolve_branch() {
    local url="$1"
    local requested="$2"

    info "Querying remote branches at $url"
    local remote_info
    remote_info="$(git ls-remote --heads --symref "$url" 2>/dev/null)" \
        || fail "Could not reach remote: $url"

    # Check whether the requested branch exists on the remote.
    if echo "$remote_info" | grep -qE "refs/heads/${requested}$"; then
        info "Branch '$requested' found on remote"
        echo "$requested"
        return 0
    fi
    ...
    # Try to read the remote HEAD symbolic ref (the true default branch).
    ...
    # Last resort: list all available branches for the user and bail out.
    local branches
    branches="$(
        echo "$remote_info" \
            | awk '/\trefs\/heads\// { sub(/.*refs\/heads\//, ""); print "  " $0 }'
    )"
    fail "Branch '$requested' not found and remote HEAD is ambiguous.
Available branches:
${branches}
Set DEBIAN_BRANCH to one of the above and re-run."
}

clone_or_update_repo() {
    step "Preparing ungoogled-chromium-debian repository"

    # Resolve the branch before touching the filesystem so we fail fast with
    # a useful message rather than a cryptic git-clone error.
    DEBIAN_BRANCH="$(_resolve_branch "$DEBIAN_REPO_URL" "$DEBIAN_BRANCH")"
    ...
    if [[ ! -d "${DEBIAN_DIR}/.git" ]]; then
        ...
        info "Existing repository found — checking state (branch: $DEBIAN_BRANCH)"
        ...
        if [[ "$current_branch" == "$DEBIAN_BRANCH" && "$current_commit" == "$remote_commit" ]]; then
            # Already on the right branch and commit — preserve everything as-is.
            # This avoids re-downloading the 30 GB Chromium source tree.
            info "Repository already up to date on '$DEBIAN_BRANCH' ($current_commit) — nothing to do"
        else
            info "Repository needs updating (was: $current_commit, now: $remote_commit)"
            # reset --hard updates tracked files only.
            # git clean -xfd is intentionally omitted so the downloaded
            # Chromium source tree, .deb files, and build artefacts are
            # NOT wiped between runs.
            git -C "$DEBIAN_DIR" checkout "$DEBIAN_BRANCH"
            git -C "$DEBIAN_DIR" reset --hard "origin/${DEBIAN_BRANCH}"
            info "Tracked files reset to origin/${DEBIAN_BRANCH} — untracked files preserved"
        fi
    fi
    ...
    info "Repository ready on branch: $DEBIAN_BRANCH"
}
